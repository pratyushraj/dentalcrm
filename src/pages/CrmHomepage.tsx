...
export default function CrmHomepage() {
  ...
  return (
    <div className="min-h-screen bg-white text-[#0B2450] font-sora antialiased overflow-x-hidden selection:bg-[#0867E8] selection:text-white">
        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
          <Link to="/" className="flex items-center gap-3 group">
            <img 
              ...
              alt="CLINAZA Logo" 
              className="h-10 w-auto rounded-xl border border-slate-200 shadow-sm group-hover:scale-105 transition-transform" 
            />
            <div className="hidden sm:block">
              <span className="text-xs font-black tracking-widest text-[#0B2450] block">CLINAZA</span>
              <span className="text-[9px] font-bold tracking-wider text-[#12A8A0] block uppercase">EMI FOR BETTER HEALTH</span>
            </div>
          </Link>
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <MessageSquare size={14} className="text-[#12A8A0]" />
              WhatsApp Us
            </a>
            <a
              href="#partner-form"
      ...
      {/* SECTION 1: HERO SECTION (WHITE BG) */}
      <section className="relative pt-16 pb-20 sm:pt-24 sm:pb-28 px-6 text-center max-w-5xl mx-auto space-y-8 z-10">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-[#F5F9FC] border border-blue-100 rounded-full text-xs font-bold tracking-wider text-[#0867E8] shadow-sm">
          <Building2 className="h-4 w-4 text-[#12A8A0]" /> 
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center pt-2">
          ...
          >
            <MessageSquare className="h-4 w-4 text-[#12A8A0]" />
            WhatsApp Us
          </a>
        </div>
        ...
        <p className="text-xs text-slate-500 font-medium max-w-xl mx-auto pt-2">
          ...
          <span className="italic text-[11px] text-slate-400">Financing handled by our lending partners. Eligibility and approval subject to lender policies.</span>
        </p>
      </section>

      {/* SECTION 2: SHOW THE PROBLEM (LIGHT BLUE BG #F5F9FC) */}
      <section className="py-20 px-6 bg-[#F5F9FC] border-y border-blue-50 relative z-10">
        <div className="max-w-5xl mx-auto space-y-12">
          <div className="text-center space-y-3">
            <span className="text-[10px] font-black text-[#12A8A0] uppercase tracking-widest">THE CLINIC CHALLENGE</span>
          ...
          <div className="bg-white border border-blue-100 p-6 rounded-2xl text-center shadow-sm">
            <p className="text-sm sm:text-base font-bold text-[#0B2450]">
              💡 <span className="text-[#12A8A0]">Clinaza</span> helps your clinic offer a financing option at the point of treatment.
            </p>
          </div>
        </div>
      </section>

      {/* SECTION 3: HOW CLINAZA WORKS (WHITE BG) */}
      <section className="py-20 px-6 max-w-6xl mx-auto relative z-10 space-y-12">
        <div className="text-center space-y-3">
          <span className="text-[10px] font-black text-[#0867E8] uppercase tracking-widest">STEP-BY-STEP WORKFLOW</span>
          <h2 className="text-2xl sm:text-4xl font-black text-[#0B2450]">How Clinaza Works For Your Clinic</h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {steps.map((step, idx) => (
            <div key={idx} className="bg-[#F7FAFC] border border-slate-200/70 p-6 rounded-3xl space-y-4 text-left relative shadow-sm">
              <span className="text-3xl font-mono font-black text-[#12A8A0] block">{step.num}</span>
      ...
      {/* SECTION 4: TREATMENT CATEGORIES (LIGHT GREY/BLUE BG #F7FAFC) */}
      <section className="py-20 px-6 bg-[#F7FAFC] border-y border-slate-200/60 relative z-10">
        <div className="max-w-5xl mx-auto space-y-12">
          <div className="text-center space-y-3">
            <span className="text-[10px] font-black text-[#12A8A0] uppercase tracking-widest">HIGH-VALUE HEALTHCARE PROCEDURES</span>
              ...
              <ul className="space-y-3">
                {dentalTreatments.map((t, idx) => (
                  <li key={idx} className="flex items-center gap-2.5 text-xs text-slate-700 font-medium">
                    <CheckCircle2 size={16} className="text-[#12A8A0] shrink-0" />
                    <span>{t}</span>
                  </li>
                ))}
              </ul>
            ...
            <div className="bg-white border border-slate-200 p-8 rounded-3xl space-y-6 text-left shadow-sm">
              ...
              <ul className="space-y-3">
                {healthcareTreatments.map((t, idx) => (
                  <li key={idx} className="flex items-center gap-2.5 text-xs text-slate-700 font-medium">
                    <CheckCircle2 size={16} className="text-[#0867E8] shrink-0" />
                    <span>{t}</span>
                  </li>
                ))}
              </ul>
      ...
      {/* SECTION 5: WHY CLINICS PARTNER WITH CLINAZA (WHITE BG) */}
      <section className="py-20 px-6 max-w-5xl mx-auto relative z-10 space-y-12">
        <div className="text-center space-y-3">
          <span className="text-[10px] font-black text-[#12A8A0] uppercase tracking-widest">CLINIC PARTNER ADVANTAGE</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {clinicBenefits.map((b, idx) => (
            <div key={idx} className="bg-[#F7FAFC] border border-slate-200/80 p-6 rounded-3xl flex items-start gap-3 text-left shadow-sm">
              <Check className="text-[#12A8A0] h-5 w-5 shrink-0 mt-0.5" />
        ...
        <div className="bg-gradient-to-r from-[#0B2450] to-[#0867E8] p-8 sm:p-12 rounded-3xl text-center space-y-6 text-white shadow-xl">
          ...
          <a
            href="#partner-form"
            className="inline-flex items-center gap-2 px-8 py-4 bg-[#12A8A0] hover:bg-[#0f968f] text-white text-xs font-black uppercase tracking-widest rounded-2xl transition-all shadow-lg"
          >
      ...
      {/* SECTION 6: TRUST & TRANSPARENCY SECTION (LIGHT GREY BG #F5F9FC) */}
      <section className="py-16 px-6 bg-[#F5F9FC] border-y border-slate-200/60 relative z-10">
        <div className="max-w-4xl mx-auto bg-white border border-slate-200 p-8 rounded-3xl space-y-4 text-left shadow-sm">
          <div className="flex items-center gap-2 text-[#12A8A0]">
            <ShieldCheck size={20} />
      ...
      {/* SECTION 7: PARTNER REGISTRATION FORM (WHITE BG WITH LIGHT CARD) */}
      <section id="partner-form" className="py-20 px-6 max-w-3xl mx-auto relative z-10">
        <div className="bg-[#F7FAFC] border border-slate-200 rounded-3xl p-6 sm:p-12 shadow-xl space-y-8 text-left">
          <div className="text-center space-y-2">
            <span className="text-[10px] font-black text-[#12A8A0] uppercase tracking-widest">GET STARTED</span>
          ...
          {submitted ? (
            <div className="bg-white border border-[#12A8A0]/40 p-8 rounded-2xl text-center space-y-3 shadow-sm">
              <CheckCircle2 size={40} className="text-[#12A8A0] mx-auto" />
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Clinic / Hospital Name *</label>
                  <input
                    type="text"
                    required
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Doctor / Administrator Name *</label>
                  <input
                    type="text"
                    required
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Phone / WhatsApp Number *</label>
                  <input
                    type="tel"
                    required
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Treatment Categories Offered</label>
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Approx. Monthly Patient Volume</label>
                <select
              ...
              <button
                ...
                className="w-full py-4 bg-[#0867E8] hover:bg-[#0756C7] text-white font-black text-xs uppercase tracking-widest rounded-xl transition-all shadow-lg shadow-[#0867E8]/20 flex items-center justify-center gap-2"
              >
              ...
              <p className="text-center text-xs text-slate-500 font-mono">
                Or WhatsApp us directly: <a href="https://wa.me/917292984244" target="_blank" rel="noopener noreferrer" className="text-[#12A8A0] font-bold underline">7292984244</a>
              </p>
            </form>
          )}
        </div>
      ...
      <footer className="border-t border-slate-200 py-12 px-6 bg-white text-center sm:text-left">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-3">
            <img src="/assets/clinaza-logo.jpg" alt="CLINAZA Logo" className="h-8 w-auto rounded-lg border border-slate-200" />
            <div>
              <span className="text-xs font-black uppercase tracking-widest text-[#0B2450] block">CLINAZA</span>
              <span className="text-[8px] font-bold text-[#12A8A0] block uppercase">EMI FOR BETTER HEALTH</span>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 text-[10px] text-slate-500 font-mono uppercase tracking-widest">
            ...
            <Link to="/yourdentist/blog" className="hover:text-[#12A8A0] transition-colors underline">Patient Guides</Link>
